...
const NAT_GATEWAY_SECTIONS = [
  ...
  {
    id: 'internet-gateway-vs-nat-gateway',
    title: 'Internet Gateway vs NAT Gateway',
    content:
      "Two side-by-side VPCs showing how a **Public EC2** reaches the internet directly versus how a **Private EC2** reaches it indirectly.\n\n**Left side — Public Subnet (direct, two-way access):** VPC → Internet Gateway → Router → NACL → Public Subnet → Route Table → Security Group → Public EC2. The arrow between the Internet Gateway and the Router is **bidirectional** — a Public EC2 has a direct route through the Internet Gateway, so it can both send and receive traffic straight from the internet.\n\n**Right side — Private Subnet (indirect, one-way outbound access):** VPC → Internet Gateway → Router → NACL → Public Subnet (containing the **NAT Gateway** instead of an EC2 instance) → NACL → Private Subnet → Route Table → Security Group → Private EC2. The arrow only flows **one way** — up from the Private EC2, through the NAT Gateway, to the Router. A Private EC2 has no public IP and no direct internet route; its outbound traffic is routed through the NAT Gateway (which itself lives inside a Public Subnet) before ever reaching the Internet Gateway.\n\n**Internet Gateway (IGW):** attached to the VPC — the door between the VPC and the internet. Enables direct, **two-way** (in and out) connectivity for resources with a public IP, like a Public EC2 in a Public Subnet.\n\n**NAT Gateway:** sits inside a Public Subnet and provides **one-way, outbound-only** internet access for resources in a Private Subnet. A Private EC2 has no public IP and can't be reached directly from the internet — but it can still reach out for software updates, third-party APIs, and other external services.\n\n**Route Tables:** each subnet has its own route table (shown as the `x.x.x` rows) — the public subnet's table routes internet-bound traffic to the Internet Gateway; the private subnet's table routes it to the NAT Gateway instead.\n\n**NACLs (Network ACLs):** a stateless, subnet-level firewall — each subnet gets its own NACL (red for the public subnet, green for the private subnet in this diagram).\n\n**Security Groups:** a stateful, instance-level firewall wrapping each EC2 instance (the Public EC2 and Private EC2 each sit inside their own Security Group).\n\n**Quick comparison:**\n- **Internet Gateway** — two-way (in & out) internet access; used by resources in a Public Subnet with a public IP.\n- **NAT Gateway** — one-way (outbound only) internet access; used by resources in a Private Subnet with no public IP, so nothing outside can initiate a connection to them.\n\n**Key takeaway:** Public EC2 → Internet Gateway = direct, two-way access. Private EC2 → NAT Gateway → Internet Gateway = outbound-only access — the private resource stays unreachable from the internet while still being able to fetch updates and call external services.",
    image: '/aws-notes/internet-gateway-vs-nat-gateway.jpg',
    imageAlt:
      'Internet Gateway vs NAT Gateway — two side-by-side VPC diagrams. Left: a Public Subnet path where a Public EC2 (inside a Security Group, behind a Route Table and NACL) connects bidirectionally through an Internet Gateway and Router straight to the Internet. Right: a Private Subnet path where a Private EC2 (inside a Security Group, behind a Route Table and its own NACL) sends outbound-only traffic up through a NAT Gateway (sitting in the Public Subnet, behind its own NACL) and Router, out through the same Internet Gateway to the Internet — with no return path back to the Private EC2.',
  },
];
