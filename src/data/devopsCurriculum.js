...
const DEVOPS_SECTION_MAP = {
  ...
  'Jenkins Pipelines': [
    ...
    {
      id: 'jenkins-cicd-pipeline-end-to-end',
      title: 'Jenkins CI/CD Pipeline (End to End)',
      content:
        "Jenkins is an open-source automation server used to implement CI/CD pipelines. It helps automate **Build, Test, Deploy, and Delivery** of applications.\n\n**1. End-to-end pipeline overview** — seven stages: ① Code Commit → ② Source Code Management (Git) → ③ Jenkins Build → ④ Test (unit test, code quality, security scan) → ⑤ Artifact Repository (Nexus/Artifactory) → ⑥ Deploy to Environments (Dev, QA, Staging, Prod) → ⑦ Monitoring & Notifications — with feedback/triggers on changes looping back to Code Commit.\n\n**2. Pipeline flow, step by step:**\n1. **Code Commit** — a developer writes code and pushes it to a Git repository (GitHub/GitLab/Bitbucket).\n2. **Webhook Trigger** — on every push/merge, Git sends a webhook request to Jenkins to start the build.\n3. **Jenkins Build (Continuous Integration)** — Jenkins fetches the latest code, compiles the application, runs unit tests, performs code quality analysis (SonarQube), and performs a security scan (OWASP Dependency Check).\n4. **Artifact Creation** — if the build & tests succeed, Jenkins creates an artifact (JAR/WAR/Docker image) and stores it in the artifact repository.\n5. **Artifact Repository** — artifacts are stored in Nexus/Artifactory for versioning and reuse.\n6. **Deploy to Environments (Continuous Delivery/Deployment)** — Jenkins deploys the artifact to different environments in sequence: Dev → QA → Staging → Production (manual approval can be added before Production). Deployment can be done using scripts, Docker, Kubernetes, Ansible, or any deployment tool.\n7. **Monitoring & Notifications** — after deployment, Jenkins sends notifications (Email/Slack/MS Teams), and the application is monitored for health.\n\n**3. Jenkins pipeline — high-level architecture:** Developer pushes code → Git Repository → webhook → Jenkins Server (Checkout Code → Build → Test → Quality & Security Scan → Package/Artifact) → Artifact Repository → Deploy to Environments (Dev → QA → Staging → Prod) → Monitoring & Alerts, which feed back to the developer.\n\n**4. Jenkins key features:**\n- Open source & extensible (plugins).\n- Easy to configure pipelines.\n- Supports all major tools & technologies.\n- Distributed builds.\n- Scalable.\n- Great community support.\n- Integrates with almost everything.\n\n**Key takeaway:** Jenkins automates the complete software delivery process and helps teams deliver quality software faster and reliably.",
      image: '/devops-notes/jenkins-cicd-pipeline-end-to-end.jpg',
      imageAlt:
        'Jenkins CI/CD Pipeline (End to End) — Jenkins is an open source automation server used to implement CI/CD pipelines, automating Build, Test, Deploy and Delivery. 1. End to end pipeline overview: 7-stage diagram (Code Commit, Source Code Management/Git, Jenkins Build, Test with unit test/code quality/security scan, Artifact Repository via Nexus/Artifactory, Deploy to Environments across Dev/QA/Staging/Prod, Monitoring & Notifications), with feedback/trigger on changes looping back. 2. Pipeline flow step by step: Code Commit, Webhook Trigger, Jenkins Build (Continuous Integration: fetch code, compile, unit tests, code quality analysis via SonarQube, security scan via OWASP Dependency Check), Artifact Creation (jar/war/docker image), Artifact Repository, Deploy to Environments (Continuous Delivery/Deployment: Dev to QA to Staging to Production with manual approval, using scripts/Docker/Kubernetes/Ansible), Monitoring & Notifications (Email/Slack/MS Teams, health monitoring). 3. Jenkins pipeline high level architecture diagram: Developer pushes code to Git Repository via webhook to Jenkins Server (Checkout Code, Build, Test, Quality & Security Scan, Package/Artifact) to Artifact Repository to Deploy to Environments (Dev to QA to Staging to Prod) to Monitoring & Alerts. 4. Jenkins key features: open source & extensible with plugins, easy to configure pipelines, supports all major tools & technologies, distributed builds, scalable, great community support, integrates with almost everything; ending with the key takeaway that Jenkins automates the complete software delivery process and helps teams deliver quality software faster and reliably',
    },
  ],
  ...
  'Linux Process Management': [
    ...
    {
      id: '20-linux-commands-cheat-sheet',
      title: '20 Must-Know Linux Commands',
      content:
        "A quick-reference table of the 20 Linux commands every DevOps engineer reaches for daily — command, purpose, and a concrete example:\n\n1. `ps` — view running processes. Example: `ps -ef`\n2. `top` — monitor system processes in real-time. Example: `top`\n3. `grep` — search for patterns in files. Example: `grep \"ERROR\" app.log`\n4. `find` — search for files and directories. Example: `find / -name \"*.log\"`\n5. `awk` — pattern scanning and processing. Example: `awk '{print $1}' file.txt`\n6. `sed` — stream editor for filtering and replacing text. Example: `sed 's/dev/prod/g' file.txt`\n7. `df` — disk space usage information. Example: `df -h`\n8. `du` — check directory space usage. Example: `du -sh /var/log`\n9. `free` — display memory usage. Example: `free -m`\n10. `netstat` — show network connections and listening ports. Example: `netstat -tulpn`\n11. `ssh` — secure login to remote machine. Example: `ssh user@hostname`\n12. `tail` — view last lines of a file (real-time with `-f`). Example: `tail -f app.log`\n13. `chmod` — change file permissions. Example: `chmod 755 script.sh`\n14. `chown` — change file ownership. Example: `chown user:group file.txt`\n15. `wget` — download files from web. Example: `wget https://example.com/file.zip`\n16. `curl` — transfer data from or to a server. Example: `curl -I https://example.com`\n17. `head` — show first lines of a file. Example: `head -10 file.txt`\n18. `sort` — sort lines of text files. Example: `sort file.txt`\n19. `uniq` — remove duplicate lines. Example: `sort file.txt | uniq`\n20. `xargs` — build and execute command lines from input. Example: `cat list.txt | xargs rm`",
      image: '/devops-notes/20-linux-commands-cheat-sheet.jpg',
      imageAlt: '20 Must-Know Linux Commands cheat sheet — a three-column table (COMMAND, PURPOSE, EXAMPLE) listing: ps (view running processes, ps -ef), top (monitor system processes in real-time, top), grep (search for patterns in files, grep "ERROR" app.log), find (search for files and directories, find / -name "*.log"), awk (pattern scanning and processing, awk \'{print $1}\' file.txt), sed (stream editor for filtering and replacing text, sed \'s/dev/prod/g\' file.txt), df (disk space usage information, df -h), du (check directory space usage, du -sh /var/log), free (display memory usage, free -m), netstat (show network connections and listening ports, netstat -tulpn), ssh (secure login to remote machine, ssh user@hostname), tail (view last lines of a file real-time with -f, tail -f app.log), chmod (change file permissions, chmod 755 script.sh), chown (change file ownership, chown user:group file.txt), wget (download files from web, wget https://example.com/file.zip), curl (transfer data from or to a server, curl -I https://example.com), head (show first lines of a file, head -10 file.txt), sort (sort lines of text files, sort file.txt), uniq (remove duplicate lines, sort file.txt | uniq), xargs (build and execute command lines from input, cat list.txt | xargs rm)',
    },
    {
...
const LINUX_TROUBLESHOOTING_SECTIONS = [
  ...
  {
    id: "high-cpu-vs-high-memory-vs-disk-full",
    title: "Linux Troubleshooting: High CPU vs High Memory vs Disk Full",
    content:
      "Three of the most common Linux production incidents, side by side.\n\n**1. High CPU**\n\n**Symptoms:** system is slow, high load average, applications not responding, fan noise increases.\n\n**Commands to check:** `top` (real-time process view), `htop` (interactive process view), `ps aux --sort=-%cpu | head -n 10` (top CPU-consuming processes), `pidstat -u 1` (CPU usage by process), `uptime` (load average), `mpstat -P ALL 1` (CPU usage per core), `sar -u 1 5` (CPU usage report).\n\n**What to look for:** processes using high %CPU, load average > number of CPU cores, high %user or %system time.\n\n**How to fix:** identify the high-CPU process, kill it if required (`kill -9 <PID>`), optimize the application or job, add more CPU if required, check for infinite loops or heavy queries.\n\n**Prevention tips:** monitor regularly (top/htop), set up alerts, optimize code and cron jobs, use caching where possible.\n\n**2. High Memory**\n\n**Symptoms:** system is slow, applications crashing, OOM Killer in logs, swap usage is high.\n\n**Commands to check:** `free -h` (memory usage), `top` (%MEM column), `ps aux --sort=-%mem | head -n 10` (top memory-using processes), `smem -r` (detailed memory usage), `vmstat 1` (virtual memory stats), `swapon --show` (swap usage), `dmesg | grep -i oom` (OOM Killer logs), `cat /proc/meminfo` (detailed memory info), `sar -r 1 5` (memory usage report).\n\n**What to look for:** used memory is very high, processes using high %MEM, swap usage is very high, OOM Killer messages in dmesg.\n\n**How to fix:** identify the high-memory process, kill it if required (`kill -9 <PID>`), clear caches if needed (`sync; echo 3 > /proc/sys/vm/drop_caches`), increase RAM or add swap (`fallocate -l 2G /swapfile && mkswap /swapfile && swapon /swapfile`).\n\n**Prevention tips:** monitor memory usage, set memory limits for apps/containers, optimize application memory usage, use swap as a safety net.\n\n**3. Disk Full**\n\n**Symptoms:** \"No space left on device\" errors, applications not writing logs/files, services failing, system slow or unresponsive.\n\n**Commands to check:** `df -h` (disk usage), `du -sh /* | sort -hr` (find large directories), `du -sh /var/* | sort -hr` (check /var usage), `lsblk` (disk details), `iostat -xh 1` (disk I/O), `df -i` (inode usage), `lsof | grep deleted` (find deleted files still open).\n\n**What to look for:** disk usage 90% or more, large files or directories, inode usage full (`df -i`), high I/O wait (`iostat`).\n\n**How to fix:** find large files/directories and remove or archive them, clean logs (`journalctl --vacuum-time=7d`, `logrotate -f /etc/logrotate.conf`), remove old packages/cache (`apt clean` on Debian/Ubuntu, `yum clean all` on RHEL/CentOS), remove unwanted files (`rm -rf /tmp/*`), add more disk space if required.\n\n**Prevention tips:** monitor disk usage, set up log rotation, clean up old files periodically, keep at least 20%+ free space.\n\n**Best practice, across all three:** Monitor → Analyze → Fix → Optimize.",
    code: "# ── 1. HIGH CPU ──────────────────────────────\ntop\nhtop\nps aux --sort=-%cpu | head -n 10\npidstat -u 1\nuptime\nmpstat -P ALL 1\nsar -u 1 5\nkill -9 <PID>\npgrep -f <process>\nnice -n 10 <command>\nrenice -10 -p <PID>\nstress --cpu 4\nvmstat 1\n\n# ── 2. HIGH MEMORY ───────────────────────────\nfree -h\nps aux --sort=-%mem | head -n 10\nsmem -r\nvmstat 1\nswapon --show\ndmesg | grep -i oom\ncat /proc/meminfo\nsar -r 1 5\nkill -9 <PID>\nsync; echo 3 > /proc/sys/vm/drop_caches\nfallocate -l 2G /swapfile && mkswap /swapfile && swapon /swapfile\npmap -x <PID>\nnumastat\ncat /proc/<PID>/status\nsysctl vm.overcommit_memory\njournalctl -k | grep -i oom\n\n# ── 3. DISK FULL ─────────────────────────────\ndf -h\ndu -sh /* | sort -hr\ndu -sh /var/* | sort -hr\nlsblk\niostat -xh 1\ndf -i                              # inode usage\nlsof | grep deleted\njournalctl --vacuum-time=7d\nlogrotate -f /etc/logrotate.conf\napt clean                          # Debian/Ubuntu\nyum clean all                      # RHEL/CentOS\nrm -rf /tmp/*\nfind / -xdev -type f -size +1G -exec ls -lh {} \\;\nncdu /\nwatch -n 5 df -h\ntune2fs -l /dev/sda1 | grep \"Inode count\"\nfstrim -av\n\n# Best practice: Monitor -> Analyze -> Fix -> Optimize",
    image: "/devops-notes/linux-troubleshooting-cpu-memory-disk.jpg",
    imageAlt:
      "Linux Troubleshooting: High CPU vs High Memory vs Disk Full — three side-by-side panels. High CPU (symptoms, commands to check like top/htop/ps aux --sort=-%cpu/pidstat/uptime/mpstat/sar, what to look for, how to fix including kill -9, prevention tips, more useful commands like pgrep/nice/renice/stress/vmstat). High Memory (symptoms, commands to check like free -h/ps aux --sort=-%mem/smem/vmstat/swapon/dmesg oom/proc meminfo/sar, what to look for, how to fix including clearing caches and adding swap, prevention tips, more useful commands like pmap/numastat/proc status/sysctl/journalctl). Disk Full (symptoms, commands to check like df -h/du -sh/lsblk/iostat/inode usage/lsof deleted, what to look for, how to fix including cleaning logs/removing packages/removing unwanted files, prevention tips, more useful commands like find/ncdu/watch/tune2fs/fstrim). Ends with the best practice: Monitor -> Analyze -> Fix -> Optimize.",
  },
];
