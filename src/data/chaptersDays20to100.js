...
export const chaptersDays20to100 = [
  ...
  },
  {
    ...
    "sections": [
      ...
      {
        "id": "elasticsearch-and-search",
        "title": "Elasticsearch & Inverted Index — Full-Text Search, BM25, and Hybrid Search",
        "content": "**1. The Search Problem.** When users type a search query, they expect relevant results instantly — even for partial words, typos, or phrase matches. Doing this with a SQL `LIKE '%query%'` forces a full table scan every time, which is unacceptably slow at scale. Elasticsearch solves this by pre-building an **inverted index** at write time so reads are O(1).\n\n**2. Inverted Index.** An inverted index maps \"word → list of documents\" instead of \"document → words\". Example: Doc1 = \"Elasticsearch is fast\", Doc2 = \"Redis is fast\". The inverted index for `fast` → [Doc1, Doc2]. A search for `fast` looks up the pre-built map and returns matching IDs instantly — no scanning. This is the same structure used by Google, Lucene, and every modern search engine.\n\n**3. Analyzer Pipeline.** Before inserting text into the index, Elasticsearch runs each string through an **analyzer pipeline**: (1) **Tokenizer** — splits raw text into tokens: `\"Elasticsearch is FAST!\"` → `[\"Elasticsearch\", \"is\", \"FAST\"]`. (2) **Lowercase filter** — normalises tokens: `[\"elasticsearch\", \"is\", \"fast\"]`. (3) **Stop-word filter** — removes noise words (`the`, `is`, `a`, `an`, etc.): `[\"elasticsearch\", \"fast\"]`. The result is what actually lands in the inverted index. The same pipeline runs on the query string at search time, so matching is consistent.\n\n**4. Positions — Phrase Search.** Elasticsearch can optionally store the **position** of each token within a document: `\"Elasticsearch is fast\"` → `{elasticsearch: 0, is: 1, fast: 2}`. Positions enable phrase matching — `match_phrase: \"elasticsearch fast\"` only matches if these two words appear adjacent and in that order. Without positions, you only know a document contains the word, not where.\n\n**5. TF-IDF and BM25 Scoring.** Elasticsearch ranks results by **relevance score**, not just presence. Two concepts drive this: **Term Frequency (TF)** — how often the search term appears in this document (more occurrences → higher score). **Inverse Document Frequency (IDF)** — how rare the term is across all documents (rare terms carry more signal). The modern algorithm is **BM25** (Best Match 25), a refinement of TF-IDF that also accounts for document length — a short document with many occurrences outscores a long document with the same raw count.\n\n**6. Boosting — Field Weights.** Not all fields are equally important. Elasticsearch lets you boost specific fields: `{ \"multi_match\": { \"query\": \"shoes\", \"fields\": [\"title^3\", \"description^1\", \"tags^2\"] } }`. A match in `title` (boost 3) is three times more valuable than a match in `description` (boost 1). This is how product search surfaces title matches above body text without changing the schema.\n\n**7. Fuzzy Search.** Typos are inevitable. Elasticsearch handles them via **fuzzy matching** based on **Levenshtein edit distance** — the minimum single-character edits (insert, delete, substitute) to transform one string into another. `\"Elastisearch\"` (one typo) has edit distance 1 from `\"Elasticsearch\"` and matches at `fuzziness: 1`. For longer words, `fuzziness: 2` catches more typos at the cost of more false positives.\n\n**8. Elasticsearch vs MongoDB.**\n\n| Feature | Elasticsearch | MongoDB |\n|---|---|---|\n| Primary use | Full-text search & analytics | General-purpose document store |\n| Data structure | Inverted index | B-tree index |\n| Query type | match, term, fuzzy, phrase | find, aggregate |\n| Ranking | BM25 relevance scoring built-in | None |\n| Best for | Search, log analytics, autocomplete | CRUD, real-time reads/writes |\n\nMany systems write to both: MongoDB as the source of truth, Elasticsearch for search.\n\n**9. Hybrid Search (Lexical + Vector).** BM25 finds keyword matches. **Vector search** (semantic search) encodes text as a high-dimensional embedding and finds semantically similar documents even when the words differ — `\"cheap shoes\"` finds `\"affordable footwear\"`. **Hybrid search** combines both: `final_score = (1 − alpha) × BM25_score + alpha × vector_score`. In Elasticsearch this is done using the `knn` query alongside a `match` query with `rrf` (reciprocal rank fusion). Most modern search products — e-commerce, docs, support — use hybrid search.\n\n**10. Real-World Use Cases.** Elasticsearch powers: e-commerce product search (Amazon-style), application log analytics (the 'E' in ELK Stack — Elasticsearch, Logstash, Kibana), autocomplete and type-ahead, full-text search across documents and wikis, observability dashboards, and real-time fraud detection via log queries.\n\n📎 [Notion Notes — Lecture 29: Elasticsearch & Bloom Filter](https://app.notion.com/p/Lecture29-Elastic-Search-and-Bloom-filter-3c4a9af81c98800a8dc9f4abb108c884)"
      },
      {
        "id": "bloom-filter",
        "title": "Bloom Filter — Probabilistic Membership Queries at Scale",
        "content": "**1. The Membership Problem.** You have a very large set and need to answer one question: *is this element in the set?* Real examples: Instagram checks if a username is taken before a new user registers. Chrome checks if a URL is on the known-malware list before loading it. Cassandra checks if a key exists in an SSTable before a costly disk read. A naive **hash set** gives O(1) lookups and exact answers — but for 2 billion usernames at ~10 bytes each plus 40-byte hash pointers, you need ~**100 GB of RAM**. Too expensive for a membership-only query.\n\n**2. What is a Bloom Filter?** A Bloom filter is a **space-efficient probabilistic data structure**. Instead of storing elements, it uses a **bit array of size m** (all bits start at 0) and **k independent hash functions**. It uses a fraction of the memory of a hash set, with one trade-off: it can produce **false positives** (says \"probably present\" when the element is absent). It **never produces false negatives** (if it says \"absent\", the element is definitely not there).\n\n**3. Inserting an Element.** To insert element X: run X through all k hash functions to get k positions in the bit array — `h1(X)`, `h2(X)`, … `hk(X)` — and set each of those bits to 1. Example with k=3 and m=10: inserting `\"alice\"` → set bits 2, 5, 9. Inserting `\"bob\"` → set bits 1, 5, 7. Bit 5 was already set by `\"alice\"` — bits overlap and that is expected.\n\n**4. Querying an Element.** To check if element Y is in the set: run Y through all k hash functions to get positions `h1(Y)` … `hk(Y)`. If **any** of those bits is 0 → element is **definitely not** in the set. If **all** of those bits are 1 → element is **probably** in the set (those bits could have been set by other elements — a false positive). Query is O(k) time, independent of how many elements the filter holds.\n\n**5. False Positives vs False Negatives.**\n- **False Negative (impossible)** — the filter says \"absent\" but the element IS present. Cannot happen: inserting an element always sets its k bits and those bits are never unset.\n- **False Positive (possible)** — the filter says \"probably present\" but the element is NOT present. The k positions for a new query happen to all be 1 because other elements set them. The false positive rate is controlled by m and k.\n\n**6. Sizing Math.** Two formulas govern the space-accuracy tradeoff:\n- **Array size:** `m = -(n × ln p) / (ln 2)²` where n = number of elements, p = desired false positive rate.\n- **Optimal hash count:** `k = (m / n) × ln 2 ≈ log₂(1 / p)`.\n\nSmaller m → more bit collisions → higher false positive rate. More hash functions (larger k) fills the array faster but also gives stronger evidence for true positives. There is a sweet spot for k at any given m and n.\n\n**7. Worked Example — Instagram Usernames.** 2 billion registered usernames, target 1% false positive rate (p = 0.01):\n- **Array size:** `m = -(2,000,000,000 × ln 0.01) / (ln 2)² = -(2B × −4.605) / 0.4805 ≈ 19.2 billion bits ≈ 2.4 GB`.\n- **Hash functions:** `k = log₂(1 / 0.01) = log₂(100) ≈ 6.64 → 7 functions`.\n\nA hash set for 2B usernames ≈ 20 GB. The Bloom filter uses **2.4 GB** — less than 12% of the memory — with only a 1% chance of a false positive (handled by a database fallback: if the filter says \"probably taken\", confirm with a real DB lookup).\n\n**8. Production Details.** Real implementations use **double hashing** to derive all k positions from just two hash computations: `hi(x) = h1(x) + i × h2(x)` for i = 0 … k−1. This avoids the cost of k truly independent hash functions. They use **fast non-cryptographic hash functions** such as MurmurHash or xxHash — not SHA-256, which is designed for collision resistance, not speed. The bit array is a contiguous `uint64[]` block in memory, making cache access fast.\n\n**9. Systems That Use Bloom Filters.** Bloom filters appear throughout production:\n- **Apache Cassandra** — each SSTable has a bloom filter; reads skip the file entirely if the filter says the key is absent.\n- **LevelDB / RocksDB** — same pattern: per-file bloom filter to avoid unnecessary disk reads.\n- **Redis** — `RedisBloom` module provides a native bloom filter data type.\n- **Chrome browser** — Safe Browsing: a local bloom filter of known-malware URLs avoids a network round-trip on every page visit.\n- **Instagram** — username availability checks at registration time.\n- **CDN / web caches** — one-hit-wonder filtering: check against a bloom filter before caching an object seen only once.\n- **Ethereum blockchain** — transaction receipts include a bloom filter of event log topics for fast log lookup.\n\n**10. Limitations.** (1) **No deletion** — you cannot remove an element because its bits may be shared with other elements. Clearing a bit creates false negatives. The workaround is a **Counting Bloom Filter**, which stores a small integer per slot instead of a single bit, allowing decrements. (2) **Degrades when over-filled** — if the filter holds more than n elements it was sized for, the false positive rate rises above the target. Size with realistic growth in mind.\n\n📎 [Notion Notes — Lecture 29: Elasticsearch & Bloom Filter](https://app.notion.com/p/Lecture29-Elastic-Search-and-Bloom-filter-3c4a9af81c98800a8dc9f4abb108c884)"
      }
    ],
    "quiz": [
      {
