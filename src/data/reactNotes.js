...
function ScoreCard({ title, score, isWinner }) {
  ...
  },
  {
    ...
    group: 'patterns',
    title: 'CSS Modules',
    tagline: 'Write CSS that is scoped locally to each component — no more global class name collisions.',
    image: '/react-notes/react21.jpeg',
    tags: ['CSS Modules', 'Scoped CSS', 'Styling', 'module.css', 'className', 'Component Styling', 'No Global Conflicts'],
    notes: [
      { k: 'What & Why?', v: 'In regular CSS, class names are **global** and can collide across components. CSS Modules scope class names **locally by default** — no more naming conflicts!\n\n**Regular CSS:** `styles.css` → `.btn { color: green; }` — global, can clash.\n**CSS Modules:** `Button.module.css` → `.btn { color: green; }` — scoped locally to that file only.' },
      { k: 'How It Works', v: '1. Create a `.module.css` file.\n2. Import it into your component.\n3. Use the classes via the imported `styles` object.\n4. Class names are **locally scoped at build time** — React/bundler transforms `.btn` into a unique hash like `Button_btn__xK2pQ`.\n\n**Note:** CSS Modules work out of the box with Create React App, Vite, Next.js and other modern setups.' },
      { k: 'Button.module.css', v: '```css\n.btn {\n  background: #16a34a;\n  color: white;\n  padding: 10px 16px;\n  border: none;\n  border-radius: 6px;\n  cursor: pointer;\n  font-weight: 600;\n}\n.btn:hover {\n  background: #15803d;\n}\n```\nEach class name here is **scoped to this module only**.' },
      { k: 'Button.jsx', v: '```jsx\nimport styles from \'./Button.module.css\';\n\nexport default function Button() {\n  return (\n    <button className={styles.btn}>\n      Click Me\n    </button>\n  );\n}\n```\n`styles.btn` is the scoped class name — at build time it becomes a unique identifier.' },
      { k: 'App.jsx', v: '```jsx\nimport Button from \'./Button\';\n\nexport default function App() {\n  return (\n    <div className={styles.container}>\n      <h2>CSS Modules in Action</h2>\n      <Button />\n    </div>\n  );\n}\n```\nUse your component just like normal. The styling stays **scoped and clean**.' },
      { k: 'Quick Tip — Composing Classes', v: 'You can compose multiple classes using template literals:\n```jsx\nclassName={`${styles.btn} ${styles.large}`}\n```\nThis applies both `.btn` and `.large` scoped class names to the element.' },
      { k: 'Why Use CSS Modules?', v: '✓ No global conflicts\n✓ Scoped & maintainable\n✓ Easy to refactor\n✓ Works great with components\n✓ Zero runtime cost — scoping happens at build time' },
    ],
    theory: [
      {
        h: 'What are CSS Modules?',
        p: 'CSS Modules are a CSS file in which all class names and animation names are **scoped locally by default**. When you import a CSS Module into a JavaScript/React file, it exposes an object that maps local class names to globally unique class names generated by the build tool.\n\n**The problem CSS Modules solve:** In a large application with many components, a `.btn` class in `styles.css` is global. If two developers both write `.btn`, they clash. The second one overrides the first.\n\n**How CSS Modules fix it:** The build tool (Vite, webpack, etc.) transforms `.btn` in `Button.module.css` into something like `Button_btn__xK2pQ` — a unique, collision-proof class name that only exists in that component\'s generated output.\n\n```\nYou write:     .btn { background: #16a34a; }\nBrowser sees:  .Button_btn__xK2pQ { background: #16a34a; }\nYou use:       <button className={styles.btn}>  →  class="Button_btn__xK2pQ"\n```\n\nThis scoping is 100% done at build time — zero runtime overhead.',
      },
      {
        h: 'How to Use CSS Modules — Step by Step',
        p: '**Step 1 — Create a `.module.css` file:**\nName it `ComponentName.module.css`. The `.module` suffix tells the bundler to treat it as a CSS Module.\n\n**Step 2 — Write your CSS normally:**\n```css\n/* Button.module.css */\n.btn {\n  background: #16a34a;\n  color: white;\n  padding: 10px 16px;\n  border-radius: 6px;\n  font-weight: 600;\n}\n.btn:hover {\n  background: #15803d;\n}\n```\n\n**Step 3 — Import it as an object:**\n```jsx\nimport styles from \'./Button.module.css\';\n```\n`styles` is now an object: `{ btn: "Button_btn__xK2pQ" }`.\n\n**Step 4 — Use it via the object:**\n```jsx\n<button className={styles.btn}>Click Me</button>\n```\nThe bundler replaces `styles.btn` with the generated unique class name.',
      },
      {
        h: 'The Full Code Example',
        p: '**Button.module.css:**\n```css\n.btn {\n  background: #16a34a;\n  color: white;\n  padding: 10px 16px;\n  border: none;\n  border-radius: 6px;\n  cursor: pointer;\n  font-weight: 600;\n}\n.btn:hover { background: #15803d; }\n```\n\n**Button.jsx:**\n```jsx\nimport styles from \'./Button.module.css\';\n\nexport default function Button() {\n  return (\n    <button className={styles.btn}>\n      Click Me\n    </button>\n  );\n}\n```\n\n**App.jsx:**\n```jsx\nimport Button from \'./Button\';\n\nexport default function App() {\n  return (\n    <div>\n      <h2>CSS Modules in Action</h2>\n      <Button />\n    </div>\n  );\n}\n```\n\n**Output:** A page with "CSS Modules in Action" heading and a styled green "Click Me" button — with zero risk of its `.btn` styles leaking into any other component.',
      },
      {
        h: 'Composing Multiple Classes',
        p: 'You often need to apply more than one class to an element. With CSS Modules, use template literals or the `clsx` / `classnames` library:\n\n**Template literal approach:**\n```jsx\n<button className={`${styles.btn} ${styles.large}`}>\n  Big Button\n</button>\n```\n\n**With clsx (recommended for complex cases):**\n```jsx\nimport clsx from \'clsx\';\n\n<button className={clsx(styles.btn, isLarge && styles.large, isDisabled && styles.disabled)}>\n  Click Me\n</button>\n```\n\n`clsx` conditionally joins class names, handling `undefined` / `false` gracefully.',
      },
      {
        h: 'CSS Modules vs Other Approaches',
        p: '| Approach | Scoping | Runtime cost | Colocation | Tooling needed |\n|---|---|---|---|---|\n| **Global CSS** | None (global) | Zero | No | None |\n| **CSS Modules** | Build-time | Zero | Yes | Bundler support |\n| **Tailwind CSS** | Utility classes (no collision) | Zero | Yes | PostCSS |\n| **CSS-in-JS (styled-components)** | Runtime | Yes | Yes | Library |\n| **Inline styles** | Element-level | Zero | Yes | None |\n\nCSS Modules hit the sweet spot: **real CSS syntax + zero runtime overhead + automatic scoping**. They work out of the box in Create React App, Vite, and Next.js with no extra configuration.',
      },
      {
        h: 'Why Use CSS Modules?',
        p: '**1. No global conflicts** — `.btn` in `Button.module.css` never clashes with `.btn` in `Card.module.css`. Each is scoped to its own component.\n\n**2. Scoped & maintainable** — CSS and components live together. Delete the component, delete the CSS file — no orphaned styles.\n\n**3. Easy to refactor** — Rename a class inside `Button.module.css` and your editor\'s find/replace stays within that one file.\n\n**4. Works great with components** — The component model of React already encourages encapsulation. CSS Modules extend that encapsulation to styles.\n\n**5. Zero runtime cost** — All scoping happens during the build. The browser just sees plain class names like `Button_btn__xK2pQ`.',
      },
    ],
    snippets: [
      {
        label: 'Button.module.css + Button.jsx — the complete setup',
        code: "/* Button.module.css */\n.btn {\n  background: #16a34a;\n  color: white;\n  padding: 10px 16px;\n  border: none;\n  border-radius: 6px;\n  cursor: pointer;\n  font-weight: 600;\n  transition: background 0.2s;\n}\n.btn:hover {\n  background: #15803d;\n}\n\n// Button.jsx\nimport styles from './Button.module.css';\n\nexport default function Button({ children = 'Click Me', onClick }) {\n  return (\n    <button className={styles.btn} onClick={onClick}>\n      {children}\n    </button>\n  );\n}",
        note: 'The .btn class is scoped to Button.module.css only — even if another component also has a .btn class, they will never clash at runtime.',
      },
      {
        label: 'Composing classes — multiple variants with template literals',
        code: "/* Card.module.css */\n.card {\n  border-radius: 12px;\n  padding: 20px;\n  background: #f9fafb;\n  box-shadow: 0 2px 8px rgba(0,0,0,0.08);\n}\n.card--featured {\n  border: 2px solid #16a34a;\n  box-shadow: 0 4px 16px rgba(22,163,74,0.18);\n}\n.card--compact {\n  padding: 12px;\n}\n\n// Card.jsx\nimport styles from './Card.module.css';\n\nexport default function Card({ children, featured = false, compact = false }) {\n  const cls = [\n    styles.card,\n    featured && styles['card--featured'],\n    compact  && styles['card--compact'],\n  ].filter(Boolean).join(' ');\n\n  return <div className={cls}>{children}</div>;\n}",
        note: 'Bracket notation (styles["card--featured"]) is needed for class names that contain hyphens. For simpler cases, camelCase class names (styles.cardFeatured) avoid the brackets entirely.',
      },
      {
