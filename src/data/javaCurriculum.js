...
const JAVA_INTERNALS_SECTIONS = [
  ...
  {
    id: 'java-thread-dump-cheat-sheet',
    title: 'Java Thread Dump Cheat Sheet',
    content:
      "A **thread dump** is a snapshot of every thread's state and stack trace at one instant — the main tool for diagnosing thread-related production issues. 13 scenarios and what to look for:\n1. **High CPU Usage** — low traffic but CPU is high, an infinite loop or heavy computation, continuous polling/retry, or the same thread continuously `RUNNABLE`. Check with `top -H -p <PID>` then `jcmd <PID> Thread.print -l`.\n2. **Application Hang / Freeze** — the app is running but not responding; threads are stuck on a DB call, API call, network call, or lock. Check thread states: `BLOCKED`, `WAITING`, `TIMED_WAITING`.\n3. **Blocked Threads** — a thread is waiting to acquire a monitor lock. In the dump, look for `waiting to lock <0x123>` and find the lock holder via `locked <0x123>`.\n4. **Deadlock** — two or more threads are waiting on each other's lock; the dump reports `Found one Java-level deadlock`. **Fix:** consistent lock ordering, avoid nested locks, reduce synchronized blocks.\n5. **Slow API Response** — threads may be stuck on a DB query, external API call, lock contention, an exhausted thread pool, or long-running business logic.\n6. **Thread Pool Exhaustion** — requests are waiting in queue, APIs are timing out, all worker threads are busy, and the queue is full. Look for many threads in the same stack, all busy.\n7. **Database Connection Pool Exhaustion** — threads waiting for a connection, pool exhausted. Common stack: `HikariPool.getConnection()`. Causes: a connection leak, slow queries, a pool that's too small, or connections not closed.\n8. **External API / Network Call Stuck** — threads stuck waiting for a network response. Common stack: `SocketInputStream.read()`, `HttpClient.execute()`, `RestTemplate.exchange()` — check timeouts and retry configuration.\n9. **Too Many Threads** — high memory usage, high context switching, a slow application, thousands of threads. Causes: uncontrolled thread creation, executors created repeatedly, threads not terminating.\n10. **OutOfMemoryError: Unable to Create Native Thread** — not a heap memory issue, the JVM can't create a new thread. Check total thread count, thread leaks, the OS thread limit, and the container memory limit.\n11. **Kafka Consumer Lag** — messages aren't getting consumed, lag is increasing. Check for a stuck consumer thread, slow message processing, blocking DB/API calls, lock contention.\n12. **Scheduler / Batch Job Stuck** — a job isn't completing, the previous execution is still running, overlapping executions. Check the job thread's state, whether it's stuck in a DB/API call, or long-running processing.\n13. **Application Shutdown Not Working** — the JVM isn't stopping after shutdown, the process is still running. Check for non-daemon threads, executors not shut down, or a scheduler still active.\n\n**Thread States in Java:** `RUNNABLE` (executing or ready to run), `BLOCKED` (waiting to acquire a monitor lock), `WAITING` (waiting indefinitely for another thread), `TIMED_WAITING` (waiting for a specified time), `NEW` (created but not started), `TERMINATED` (execution completed).\n\n**Important commands:**\n- `jcmd <PID> Thread.print -l` — prints a thread dump with locked monitors.\n- `jstack -l <PID> > thread-dump.txt` — writes a thread dump to a file.\n- `top -H -p <PID>` — finds high-CPU threads.\n- `ps -eLf | grep <PID> | wc -l` — total thread count in the process.\n\n**How to analyze a thread dump:**\n1. Take 3-5 thread dumps at 5-10 second intervals.\n2. Identify threads in `BLOCKED`, `WAITING`, `TIMED_WAITING`, and `RUNNABLE` states.\n3. For `BLOCKED` threads → find the lock holder.\n4. For high CPU → find the thread with the same method in the `RUNNABLE` state across all dumps.\n5. Check the dump for a deadlock section.\n6. Check for threads waiting on DB, network, locks, sleep, or join.\n7. Identify the root cause and fix the code/configuration.\n\n**Thread Dump vs Heap Dump:** use a **thread dump** for high CPU, deadlock, blocked threads, application hang/freeze, slow API response (plus APM/logs), thread pool exhaustion, database connection pool exhaustion, external API/network calls stuck, too many threads, and `OutOfMemoryError: unable to create new native thread`. Use a **heap dump** instead for a Java heap-space OOM or a memory leak (for high GC use GC logs + JFR; for a container OOMKilled use container metrics/limits; for a JVM crash use a core dump / `hs_err_pid` file).\n\n**Production best practice:** always take multiple thread dumps (3-5 dumps, 5-10 sec apart) — a single dump can be misleading; compare dumps to find threads stuck in the same state/method; don't kill blocked threads immediately (it may corrupt application state); use proper monitoring and alerts (CPU, threads, GC, memory, DB, etc.). If the same thread is in the same method across all dumps, it's the culprit.\n\n**Interview one-line answer:** a thread dump is mainly used to diagnose thread-related production issues such as high CPU, application hang, deadlock, blocked threads, thread-pool exhaustion, stuck DB or external calls, too many threads, Kafka consumer lag, and native-thread `OutOfMemoryError`. For heap memory leaks, use a heap dump instead.\n\n**Key takeaway:** a thread dump shows what threads are doing; a heap dump shows what objects are occupying memory. Remember: right tool for the right problem — fast RCA & fix.",
    code: "jcmd <PID> Thread.print -l\njstack -l <PID> > thread-dump.txt\ntop -H -p <PID>\nps -eLf | grep <PID> | wc -l\n\n# Take 3 dumps 5s apart to spot the culprit thread\njcmd <PID> Thread.print -l > dump1.txt\nsleep 5\njcmd <PID> Thread.print -l > dump2.txt\nsleep 5\njcmd <PID> Thread.print -l > dump3.txt",
    image: '/java-notes/java-thread-dump-cheat-sheet.jpg',
    imageAlt: 'Java Thread Dump Cheat Sheet — 13 production scenarios (high CPU usage, application hang/freeze, blocked threads, deadlock, slow API response, thread pool exhaustion, database connection pool exhaustion, external API/network call stuck, too many threads, OutOfMemoryError unable to create native thread, Kafka consumer lag, scheduler/batch job stuck, application shutdown not working) each with what to look for; a Thread States in Java table (RUNNABLE, BLOCKED, WAITING, TIMED_WAITING, NEW, TERMINATED); important commands (jcmd Thread.print, jstack, top -H, ps -eLf); a 7-step how-to-analyze-a-thread-dump guide; a Thread Dump vs Heap Dump comparison table; production best practices; and an interview one-line answer.',
  },
  {
  ...
  {
    id: '20-must-know-concurrency-concepts',
    title: '20 Must-Know Concurrency Concepts',
    content:
      "A visual reference to the 20 concurrency concepts every Java developer should know:\n\n1. **Concurrency** — multiple tasks make progress by taking turns on one CPU (interleaved execution).\n2. **Concurrency vs Parallelism** — Concurrent = interleaved (one core, context switching); Parallel = simultaneous (multiple cores executing at the same time).\n3. **Processes vs Threads** — threads share memory within a process; processes are isolated from each other.\n4. **Thread Lifecycle** — `NEW` → `RUNNABLE` → `RUNNING` → `BLOCKED` → `TERMINATED`.\n5. **Race Condition** — unsynchronized access to shared data leads to incorrect results when two threads read/write simultaneously.\n6. **Mutex** — only one thread holds the mutex at a time; all others wait (mutual exclusion).\n7. **Semaphore** — allows N threads to access a resource concurrently (a counter-based lock, S=2 means 2 threads allowed).\n8. **Condition Variables** — a thread waits on a condition (CV); another thread signals it via `signal()` when ready.\n9. **Coarse vs Fine Lock** — coarse lock covers the whole data structure (simple, less parallelism); fine-grained lock covers individual elements (more parallelism, more complexity).\n10. **Reentrant Lock** — the same thread can acquire its own lock multiple times without blocking (lock count increments each time).\n11. **Try-Lock** — attempt to acquire a lock without blocking; returns immediately — proceed if acquired, do other work if busy.\n12. **CAS (Compare-And-Swap)** — atomically update a value only if it matches the expected value; if not, the operation fails (no lock needed).\n13. **Deadlock** — two threads each hold a lock the other needs — stuck forever. Fix: consistent lock ordering, avoid nested locks.\n14. **Livelock** — threads keep responding to each other but make no actual progress (step back repeatedly, no forward movement).\n15. **Signaling Pattern** — a producer `notify()`s a signal; a waiting consumer wakes up when the condition is met.\n16. **Thread Pool** — reuse a fixed set of worker threads to execute many tasks efficiently (task queue → dispatch → pool of workers).\n17. **Producer-Consumer** — producer adds to a shared buffer (`put()`); consumer removes (`take()`) — producer blocks when full, consumer blocks when empty.\n18. **Reader-Writer** — multiple readers allowed concurrently; a writer needs exclusive access (no readers or other writers).\n19. **Thread-Safe Cache** — use read-write locks (`RW Lock`) to allow concurrent reads and safe (exclusive) writes to a cache backed by a DB.\n20. **Blocking Queue** — producer blocks when the queue is full; consumer blocks when the queue is empty — coordinates producer and consumer automatically.",
    image: '/java-notes/20-must-know-concurrency-concepts.jpg',
    imageAlt: '20 Must-Know Concurrency Concepts — a 4×5 grid of illustrated panels covering: 1 Concurrency (interleaved CPU tasks), 2 Concurrency vs Parallelism (Core1 concurrent vs Core1+Core2 parallel), 3 Processes vs Threads (shared memory inside a process), 4 Thread Lifecycle (NEW→RUNNABLE→RUNNING→BLOCKED→TERMINATED), 5 Race Condition (two threads writing x=0 to x=1?), 6 Mutex (only one thread holds the lock), 7 Semaphore (S=2 allows N threads), 8 Condition Variables (wait/signal with CV), 9 Coarse vs Fine Lock (A,B,C covered by one vs three locks), 10 Reentrant Lock (lock count=2 same thread), 11 Try-Lock (acquired→proceed / busy→do other work), 12 CAS (CAS(5→6) matches mem=5 succeeds; CAS(5→7) fails), 13 Deadlock (T1 holds L1 waits L2; T2 holds L2 waits L1), 14 Livelock (step back repeatedly, no progress), 15 Signaling Pattern (PRODUCER→SIGNAL→CONSUMER waiting, notify wakes), 16 Thread Pool (TASK QUEUE dispatches to POOL W1/W2/W3), 17 Producer-Consumer (BUFFER put/take, blocks if full/empty), 18 Reader-Writer (R1/R2/R3 read allowed; W write exclusive), 19 Thread-Safe Cache (T1/T2 read, T3 write via RW Lock → DB), 20 Blocking Queue (PRODUCER blocks if full; CONSUMER blocks if empty)',
  },
];
...
const KAFKA_DEEP_DIVE_SECTIONS = [
  ...
  },
  {
    ...
    title: 'Kafka Delivery Guarantees — Overview',
    content:
      "**What happens if a message is lost? Or processed twice?** Distributed systems must choose between speed, reliability, and duplicates — that's where Kafka's **delivery guarantees** come in.\n\n**Delivery guarantees** define what Kafka promises when delivering messages from a producer to a consumer. There are three models:\n\n1. **At Most Once** — a message is delivered **zero or one time**. If a failure occurs before processing finishes, the message is lost forever. Characteristics: fastest, no retries, possible message loss.\n2. **At Least Once** — a message is delivered **one or more times**. If processing fails, Kafka delivers the message again — no data is lost, but duplicates are possible. Characteristics: reliable, retries, duplicate processing.\n3. **Exactly Once** — Kafka guarantees each message is processed **exactly one time** — no duplicates, no message loss. Requires **idempotent producers** and **Kafka transactions**.\n\n**Each model makes different trade-offs** between speed, reliability, and complexity.",
    image: '/java-notes/kafka-delivery-guarantees.png',
    imageAlt:
      "Kafka Delivery Guarantees — 10-section visual guide by Vishwanath Patil (@patilvish). 1. What are Delivery Guarantees — three models: At Most Once, At Least Once, Exactly Once. 2. At Most Once — message delivered zero or one time; if failure before processing, message is lost; Producer → Kafka → Consumer → Crash → Message Lost; Characteristics: Fastest, No retries, Possible message loss. 3. At Least Once — message delivered one or more times; crash → Retry → Consumer Again; Characteristics: Reliable, Retries, Duplicate processing. 4. Exactly Once — each message processed exactly one time; Producer → Kafka → Consumer → Processed Once checkmark; No duplicates, No message loss, requires idempotent producers and Kafka transactions. 5. Delivery Guarantee Comparison table — At Most Once (message loss: Possible, duplicates: No, performance: Fastest), At Least Once (message loss: No, duplicates: Possible, performance: High), Exactly Once (message loss: No, duplicates: No, performance: Lowest Throughput). 6. Real-World Examples — At Most Once: Metrics, Application Logs, Monitoring Data (losing one message usually not critical); At Least Once: Order Processing, Email Notifications, Inventory Updates, Audit Events (missing data unacceptable, duplicates can be handled); Exactly Once: Banking, Payments, Financial Transactions, Invoice Generation (processing same event twice causes serious problems). 7. Production Example — Without protection: Payment Event → Processed → Consumer Crash → Retry → Rs100 Charged Again cross; With Exactly Once: Payment Event → Kafka Transaction → Processed Once checkmark (customer charged only once). 8. Common Mistake — many developers assume Exactly Once is always best; it is not — adds extra coordination and overhead; At Least Once + Idempotent Consumers provides best balance. 9. Best Practices — Use At Most Once for telemetry and logs; Use At Least Once for most business events; Use Exactly Once only for critical financial workflows. 10. Key Takeaway — At Most Once: fast but messages may be lost; At Least Once: reliable but duplicates possible; Exactly Once: no loss, no duplicates, additional complexity; choose based on business requirements not just technology.",
  },
  {
    id: 'kafka-at-most-once',
    title: 'At Most Once',
    content:
      "A message is delivered **zero or one time**.\n\n**Flow:** Producer → Kafka → Consumer → Crash → Message Lost.\n\nIf a failure occurs **before processing finishes**, the message is lost forever — the consumer commits the offset before actually processing, so Kafka won't redeliver it.\n\n**Characteristics:**\n- ✅ Fastest\n- ✅ No retries\n- ❌ Possible message loss\n\n**When to use:** non-critical data where losing a message doesn't matter — metrics, application logs, monitoring data. Losing one message out of a million usually isn't critical for dashboards or telemetry.",
  },
  {
    id: 'kafka-at-least-once',
    title: 'At Least Once',
    content:
      "A message is delivered **one or more times**.\n\n**Flow:** Producer → Kafka → Consumer → Crash → Retry → Consumer Again.\n\nIf processing fails, Kafka delivers the message again. No data is lost — but **duplicates are possible** because the consumer commits the offset only *after* successful processing. If it crashes after processing but before committing, the same message is redelivered.\n\n**Characteristics:**\n- ✅ Reliable\n- ✅ Retries\n- ⚠️ Duplicate processing\n\n**When to use:** most business events where losing data is unacceptable but duplicates can be handled — order processing, email notifications, inventory updates, audit events. Consumers should be designed to be **idempotent** (processing the same message twice produces the same result).",
  },
  {
    id: 'kafka-exactly-once',
    title: 'Exactly Once',
    content:
      "Kafka guarantees each message is processed **exactly one time** — no duplicates, no message loss.\n\n**Flow:** Producer → Kafka → Consumer → Processed Once ✓\n\nThis is the strongest guarantee and requires two things:\n1. **Idempotent producers** — Kafka deduplicates retried messages using a producer ID and sequence number, so retries don't create duplicates in the broker.\n2. **Kafka transactions** — the producer wraps writes across multiple topics/partitions in a single atomic transaction; either all succeed or none do.\n\n**Characteristics:**\n- ✅ No duplicates\n- ✅ No message loss\n- ⚠️ Lowest throughput (extra coordination overhead)\n\n**When to use:** critical financial workflows — banking transactions, payments, financial reporting, invoice generation. Processing the same event twice (e.g. charging a customer twice) can cause serious, hard-to-reverse problems.",
  },
  {
    id: 'kafka-delivery-comparison',
    title: 'Delivery Guarantee Comparison',
    content:
      "| Model | Message Loss | Duplicates | Performance |\n|---|---|---|---|\n| At Most Once | Possible | No | Fastest |\n| At Least Once | No | Possible | High |\n| Exactly Once | No | No | Lowest Throughput |\n\n**Real-world use cases:**\n\n**At Most Once** — Metrics · Application Logs · Monitoring Data\nLosing one message usually isn't critical.\n\n**At Least Once** — Order Processing · Email Notifications · Inventory Updates · Audit Events\nMissing data is unacceptable. Duplicates can be handled (idempotent consumers).\n\n**Exactly Once** — Banking · Payments · Financial Transactions · Invoice Generation\nProcessing the same event twice can cause serious problems.",
  },
  {
    id: 'kafka-delivery-production-example',
    title: 'Production Example — Payment Processing',
    content:
      "A real-world payment scenario shows why the choice of guarantee matters.\n\n**Without protection (At Least Once, no idempotency):**\n1. Payment Event published\n2. Consumer processes it → ₹100 charged\n3. Consumer crashes before committing offset\n4. Kafka retries → consumer processes again\n5. **₹100 charged again ❌** — customer double-charged\n\n**With Exactly Once (Kafka Transaction):**\n1. Payment Event published\n2. Kafka Transaction wraps the processing\n3. Consumer processes → Processed Once ✓\n4. **The customer is charged only once** ✅\n\n**Common Mistake:** many developers assume Exactly Once is always the best option — **it isn't**. It adds extra coordination and overhead that reduces throughput. For many systems, **At Least Once + Idempotent Consumers** provides the best balance of reliability and performance.\n\n**Best Practices:**\n- ✅ Use **At Most Once** for telemetry and logs.\n- ✅ Use **At Least Once** for most business events.\n- ✅ Use **Exactly Once** only for critical financial workflows.\n\n**Key Takeaway:** choosing the right delivery guarantee depends on your **business requirements** — not just the technology.\n- **At Most Once** → Fast, but messages may be lost.\n- **At Least Once** → Reliable, but duplicates are possible.\n- **Exactly Once** → No loss, no duplicates — with additional complexity.",
  },
];
