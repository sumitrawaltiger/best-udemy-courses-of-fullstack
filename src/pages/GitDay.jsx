...
import { GIT_EPISODES, GIT_GROUPS, GIT_META, getGitEpisode } from '../data/gitNotes';
...
export default function GitDay() {
  const { day } = useParams();
  const [zoom, setZoom] = useState(false);
  const entry = getGitEpisode(day);
      ...
      <div className="prereq-page">
        <section className="prereq-hero">
          ...
          <p className="prereq-hero-sub">
            The Git series currently has {GIT_EPISODES.length} episode{GIT_EPISODES.length !== 1 ? 's' : ''} — more coming soon.
          </p>
          <div className="prereq-hero-actions">
            <Link to="/git" className="prereq-btn prereq-btn-primary">
  ...
  const group = GIT_GROUPS.find((g) => g.id === entry.group);
  const prev = GIT_EPISODES.find((e) => e.ep === entry.ep - 1);
  const next = GIT_EPISODES.find((e) => e.ep === entry.ep + 1);
  const progress = Math.round((entry.ep / Math.max(GIT_EPISODES.length, entry.ep)) * 100);
  ...
  return (
    <div className={`prereq-page prereq-day-page prereq-theme-${entry.group}`}>
      <nav className="prereq-daynav">
        <Link to="/git" className="prereq-daynav-btn">
          All episodes
        </Link>
        <span className="prereq-daynav-spacer" />
        {prev ? (
          <Link to={`/git/day/${prev.ep}`} className="prereq-daynav-btn">
            ← Ep {pad(prev.ep)}
          </Link>
        ) : (
        ...
        {next ? (
          <Link to={`/git/day/${next.ep}`} className="prereq-daynav-btn prereq-daynav-next">
            Ep {pad(next.ep)} →
          </Link>
        ) : (
          <Link to="/git" className="prereq-daynav-btn prereq-daynav-next">
            Back to all episodes →
          </Link>
        )}
      </nav>
      ...
      <header className="prereq-dayhead">
        <div className="prereq-dayhead-tags">
          <span className="prereq-daychip">Git</span>
        ...
        <p className="prereq-progress-label">
          Episode {entry.ep} · {GIT_META.title}
        </p>
      </header>
      ...
      <div className="prereq-daybody">
        ...
        <figure className="prereq-figure">
          ...
          <figcaption>
            Episode {pad(entry.ep)} · {entry.title} — the original one-page episode
          </figcaption>
        </figure>
      </div>
      ...
      <footer className="prereq-dayfoot">
        {next ? (
          <Link to={`/git/day/${next.ep}`} className="prereq-btn prereq-btn-primary">
