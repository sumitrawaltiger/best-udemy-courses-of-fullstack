...
import {
  ...
  NEXTJS_NOTES_GROUPS,
  NEXTJS_NOTES_META,
  getNextjsNotesEpisode,
} from '../data/nextjsNotes';
...
export default function NextjsNotesDay() {
  const { day } = useParams();
  const [zoom, setZoom] = useState(false);
  const entry = getNextjsNotesEpisode(day);
      ...
      <div className="prereq-page">
        <section className="prereq-hero">
          ...
          <p className="prereq-hero-sub">
            The Next.js Notes series currently has {NEXTJS_NOTES_EPISODES.length} episode
            {NEXTJS_NOTES_EPISODES.length !== 1 ? 's' : ''} — more coming soon.
          </p>
          <div className="prereq-hero-actions">
            <Link to="/nextjs-notes" className="prereq-btn prereq-btn-primary">
  ...
  const group = NEXTJS_NOTES_GROUPS.find((g) => g.id === entry.group);
  const prev = NEXTJS_NOTES_EPISODES.find((e) => e.ep === entry.ep - 1);
  const next = NEXTJS_NOTES_EPISODES.find((e) => e.ep === entry.ep + 1);
  const progress = Math.round(
    (entry.ep / Math.max(NEXTJS_NOTES_EPISODES.length, entry.ep)) * 100,
  );
  ...
  return (
    <div className={`prereq-page prereq-day-page prereq-theme-${entry.group}`}>
      <nav className="prereq-daynav">
        <Link to="/nextjs-notes" className="prereq-daynav-btn">
          All episodes
        </Link>
        <span className="prereq-daynav-spacer" />
        {prev ? (
          <Link to={`/nextjs-notes/day/${prev.ep}`} className="prereq-daynav-btn">
            ← Ep {pad(prev.ep)}
          </Link>
        ) : (
        ...
        {next ? (
          <Link to={`/nextjs-notes/day/${next.ep}`} className="prereq-daynav-btn prereq-daynav-next">
            Ep {pad(next.ep)} →
          </Link>
        ) : (
          <Link to="/nextjs-notes" className="prereq-daynav-btn prereq-daynav-next">
            Back to all episodes →
          </Link>
        )}
      </nav>
      ...
      <header className="prereq-dayhead">
        <div className="prereq-dayhead-tags">
          <span className="prereq-daychip">Next.js</span>
          <span className="prereq-daychip">Episode {pad(entry.ep)}</span>
          <span className="prereq-daychip">
            {group?.icon} {group?.label}
          </span>
        </div>
        <h1 className="prereq-dayhead-title">
          <span className="prereq-dayhead-num">EP {pad(entry.ep)}</span>
          {entry.title}
        </h1>
        <p className="prereq-dayhead-tagline">{entry.tagline}</p>
        <div className="prereq-progress" aria-label={`Episode ${entry.ep}`}>
          <div className="prereq-progress-bar" style={{ width: `${progress}%` }} />
        </div>
        <p className="prereq-progress-label">
          Episode {entry.ep} · {NEXTJS_NOTES_META.title}
        </p>
      </header>
      ...
      <div className="prereq-daybody">
        ...
        <figure className="prereq-figure">
          <button
            ...
            aria-label={`Zoom episode ${entry.ep}: ${entry.title}`}
          >
            <img
              src={entry.image}
              alt={`Episode ${entry.ep} — ${entry.title}`}
              loading="lazy"
              onError={(e) => {
                const f = e.currentTarget.closest('.prereq-figure');
          ...
          <figcaption>
            Episode {pad(entry.ep)} · {entry.title} — the original handwritten notes
          </figcaption>
        </figure>
      </div>
      ...
      {entry.theory?.length > 0 && (
        <section className="prereq-theory">
          ...
          <p className="prereq-theory-intro">
            Everything on the episode above, written out — so you can read it instead of squinting
            at it.
          </p>
      ...
      {zoom && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`Episode ${entry.ep} — ${entry.title}`}
          onClick={() => setZoom(false)}
        >
          <button type="button" className="prereq-lightbox-close" aria-label="Close">
            ✕
          </button>
          <img src={entry.image} alt={`Episode ${entry.ep} — ${entry.title}`} />
        </div>
      )}

      <footer className="prereq-dayfoot">
        {next ? (
          <Link to={`/nextjs-notes/day/${next.ep}`} className="prereq-btn prereq-btn-primary">
            Next · Ep {pad(next.ep)} — {next.title} →
          </Link>
        ) : (
          <Link to="/nextjs-notes" className="prereq-btn prereq-btn-primary">
            Back to all episodes →
